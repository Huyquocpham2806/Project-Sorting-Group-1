...
def insertion_sort(a: list[int]) -> tuple[list[int], int]: #book 1
    ...
    for i in range (1, n):
        ...
        if j > 0: # chỉ đếm nếu như có so sánh vì nếu j = 0 thì sẽ không có phép so sánh
            comparison += 1
...
def bubble_sort(a: list[int]) -> tuple[list[int], int]: #đưa phần tử lớn về cuối # book 2 (page 106)
    n = len(a)
    comparison = 0
    swapped = False #đây là chỗ được gọi là modified trong book 2
...
def counting_sort(a: list[int]) -> tuple[list[int], int]: #book 1
    ...
    return b_id, n - 1 # n - 1 là số phép comparsion của phép max
